...
class CategoryController {
	...
	/**
	 * Hiển thị trang quản lý danh mục (Ngành nghề + Địa điểm) cho Admin.
	 *
	 * @return void
	 */
	public function showCategories() {
		...
		$nganhNgheList = $this->categoryModel->getIndustryList();
		$diaDiemList   = $this->categoryModel->getLocationList();
	...
	/**
	 * Cập nhật một danh mục (Ngành nghề hoặc Địa điểm) từ form Admin.
	 *
	 * @return void
	 */
	public function updateCategory() {
		...
		if ($oldMa && $newMa && $ten && in_array($loai, ['nganhnghe', 'diadiem'])) {
			if ($this->categoryModel->updateCategoryItem($oldMa, $newMa, $ten, $loai)) {
	...
	/**
	 * Xóa một danh mục theo mã (dùng chung cho ngành nghề và địa điểm).
	 *
	 * @return void
	 */
	public function deleteCategory() {
		...
		if ($maDanhMuc && $this->categoryModel->deleteCategoryItem($maDanhMuc)) {
