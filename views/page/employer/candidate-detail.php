...
//
// LƯU Ý: Tab "Hồ sơ ứng viên" bên dưới cần các field NgaySinh, GioiTinh,
// DiaChi trong $hoSoUngTuyen. Nếu ApplicationModel::getDetailForRecruiter()
// hiện chưa SELECT các cột này (JOIN với bảng ứng viên/user), cần bổ sung
// thì các ô tương ứng mới có dữ liệu, nếu không sẽ hiển thị "Chưa cập nhật".
...
$gioiTinhLabel = match ($hoSoUngTuyen['GioiTinh'] ?? null) {
	0, '0' => 'Nam',
	1, '1' => 'Nữ',
	default => 'Chưa cập nhật'
};

$ngaySinhLabel = !empty($hoSoUngTuyen['NgaySinh'])
	? date('d/m/Y', strtotime($hoSoUngTuyen['NgaySinh']))
	: 'Chưa cập nhật';
?>
...
<section class="py-5 bg-light">
	<div class="container">
		...
		<!-- TABS: CV chi tiết / Hồ sơ ứng viên -->
		<div class="mx-auto" style="max-width: 900px;">
			<ul class="nav nav-tabs nav-fill" id="candidateTab" role="tablist">
				<li class="nav-item" role="presentation">
					<button class="nav-link active" id="cv-tab" data-bs-toggle="tab" data-bs-target="#cv-pane" type="button" role="tab" aria-controls="cv-pane" aria-selected="true">
						<i class="fa-solid fa-file-lines me-2"></i>CV
					</button>
				</li>
				<li class="nav-item" role="presentation">
					<button class="nav-link" id="hoso-tab" data-bs-toggle="tab" data-bs-target="#hoso-pane" type="button" role="tab" aria-controls="hoso-pane" aria-selected="false">
						<i class="fa-solid fa-id-card me-2"></i>Hồ sơ ứng viên
					</button>
				</li>
			</ul>
			<div class="tab-content bg-white border border-top-0 rounded-bottom shadow-sm p-4 mb-4" id="candidateTabContent">

				<!-- TAB: CV (CHỈ XEM) -->
				<div class="tab-pane fade show active" id="cv-pane" role="tabpanel" aria-labelledby="cv-tab" tabindex="0">
					<?php if (!$cv): ?>
						<p class="text-muted mb-0 text-center py-4">Ứng viên chưa có CV trên hệ thống.</p>
					<?php else: ?>
				...
				<!-- TAB: HỒ SƠ ỨNG VIÊN (thông tin cá nhân, chỉ xem) -->
				<div class="tab-pane fade" id="hoso-pane" role="tabpanel" aria-labelledby="hoso-tab" tabindex="0">
					<h5 class="fw-bold text-primary mb-3">
					...
					<div class="row g-4">
						<div class="col-md-6">
							<p>
								...
								<?= htmlspecialchars($hoSoUngTuyen['TenUngVien'] ?: 'Chưa cập nhật') ?>
							</p>
							<p>
								<strong>Email liên hệ:</strong><br>
								<?= htmlspecialchars($hoSoUngTuyen['EmailUngVien'] ?: 'Chưa cập nhật') ?>
							</p>
							<p class="mb-0">
								<strong>Số điện thoại:</strong><br>
								<?= htmlspecialchars($hoSoUngTuyen['SdtUngVien'] ?: 'Chưa cập nhật') ?>
							</p>
						</div>
						<div class="col-md-6">
							<p>
								<strong>Ngày sinh:</strong><br>
								<?= htmlspecialchars($ngaySinhLabel) ?>
							</p>
							<p>
								<strong>Giới tính:</strong><br>
								<?= htmlspecialchars($gioiTinhLabel) ?>
							</p>
							<p class="mb-0">
								<strong>Địa chỉ:</strong><br>
								<?= htmlspecialchars($hoSoUngTuyen['DiaChi'] ?: 'Chưa cập nhật') ?>
							</p>
						</div>
					</div>
				</div>
